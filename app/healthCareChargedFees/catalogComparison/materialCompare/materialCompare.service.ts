...
export class DataTreeService{
	...
	private versionUrl =this.baseName + "version/list/CL/";/*获取版本*/
	private materialCatalogueUrl = this.baseName + "comparison/material/"; //获取材料比对目录
	private searchCompareCatalogueUrl = this.baseName +'comparison/material/'; // 搜索材料比对目录
	private autoComparisonUrl = this.baseName + "comparison/material/"; //自动比对
	private standardUrl = this.baseName + "material"; //获取右侧列表
	private getSearchRightCatalogue = this.baseName +"material/comparison/keyword" //搜索右侧目录
	private deleteCompare = this.baseName + "comparison/"; /*取消比对*/
	private onlyCompareUrl = this.baseName + "comparison/material/"; //只显示未比对
    ...
    getCompareCatalogue(code?:string,insuranceId?:any,versionId?:any,type?:any){
    	...
    	let tempUrl :string = this.materialCatalogueUrl + code + "/" +insuranceId + "/" +versionId + '/' +type+"?"+d;
    		return this.http.get(tempUrl)
    			.toPromise()
    			.then(this.extractJson)
    			.catch(this.handleError);
    ...
    onlyShowNoCompare(code?:string,flag?:any,insuranceId?:any,versionId?:any,type?:any){
    	...
    	if(flag){
    		...
    		tempUrl = tempUrl;
              console.log(1111111111112222222222233333333)
    	}
    	return this.http.get(tempUrl)
    		.toPromise()
    		.then(this.extractJson)

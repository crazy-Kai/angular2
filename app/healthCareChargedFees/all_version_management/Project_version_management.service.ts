...
import {ProjectVersionManagementSave,VersionManagementmodification} from './version_management_detail';
@Injectable()
export class ProjectVersionManagementService {
    ...
    private inversionListUrl = this.baseName + 'version/list2/XM/';//药品版本管理的列表信息
    ...
    private selectListURL = this.baseName + 'version/list2/comboBox/XM/';//药品添加中的下拉列表    
...
//根据版本管理增加数据   根据老版本新增新目录版本管理     
    addVersionManagement(projectVersionManagementSave:ProjectVersionManagementSave){
           return this.http.post(this.addVersionUrl, JSON.stringify(projectVersionManagementSave), {headers: this.headers})
                   .toPromise()
                   .then(this.extractJson)
                   .catch(this.handleError);
    }  

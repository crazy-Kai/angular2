...
import { DataTreeService } from './material.service';
...
export class MaterialComponentApp {
	...
	/*变量*/
	importTreeUrl = "/ipharmacare-distributed-yb-web/material/excel";/*导入*/
	private isShow:boolean = false;
	private isTextShow:boolean = true;
	materialTreeData:any;
	...
	templateUrl :any = "/ipharmacare-distributed-yb-web/template/download?filename=医保材料目录导入模版.xlsx";
	...
	*-------------------
	...
	getDataTree(dictValue?: string,versionId?:string){
		this.dataTreeService.getMaterialTree(dictValue,versionId)
			.then(res=> {
				if(res.code == 200){
	 ...
	 /*版本管理*/
    versionControl(){
    	let link:any= ['healthCareChargedFees/medicareCatalog/material/all_version_management/Material_version_management/'+this.insuranceId];
	...
	* 拖拽方法和拖拽错误提示
	...
	publicSaveData(fn?:any,data?:any,parentNode?:any){
			 this.dataTreeService.testIsSave('5',this.versionId)
			 .then( res =>{
			 		if(res.code == 200){
			 			if(res.data == '2'){
	...
	resetTree(node){
		if(node.node.pid){
				let parentNode  = this.tree.treeModel.getNodeById(node.node.pid),
					...
					console.log(2)
				this.materialTreeData.push(node.node);
				this.tree.treeModel.update();
			}
			if(node.to.node.data.name ){
				...
				console.log(4)
				this.materialTreeData.splice(node.to.index,1);
				this.tree.treeModel.update();
			}
	}
  	/********* 增删改查操作后更新目录树的逻辑
   ...
   addRootDirectory(node){
	 	...
	 	this.materialTreeData.push(nodeObject);
	 	this.tree.treeModel.update();
   }

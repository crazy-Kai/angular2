...
export class DataTreeService{
	 ...
	 private baseUrl = "/ipharmacare-distributed-yb-web/";
		versionUrl =this.baseUrl + "version/list/XM/";/*获取版本*/
	 	itemTreeUrl = this.baseUrl + "item/tree";/*获取节点*/
	 	addTreeUrl = this.baseUrl + "item";/*添加*/
    searchTreeUrl = this.baseUrl + "item/";/*选择节点*/
    searchByKeyWordTreeUrl=this.baseUrl + "item/treeKeyword";/*按字查询*/
    dragTreeUrl=this.baseUrl + "item/drag"; /*拖拽*/
   	reimbursementUrl = this.baseUrl + "reimbursement";/*报销分类*/
   	saveMessageUrl = this.baseUrl + "warningInfoSetting";//保存警示信息设置
    testNameUrl = this.baseUrl +'item/checkSummary';//检测同名
    testSaveUrl = this.baseUrl + 'ruleVersion/status';//点击保存检测
      
	 	 //获取项目树的所有子节点
	   getChildrenByNode(node?: any,insuranceId?:any,versionId?:any): Promise<any[]> {
	        let tempUrl: string = this.itemTreeUrl;
             if (node && node.id){
                tempUrl += "?pid=" + node.id+"&&insuranceId="+insuranceId+"&&versionId="+versionId;
             }else{
                 tempUrl += "?insuranceId="+insuranceId+"&&versionId="+versionId;
             }
    ...
    checkName(name?:string,id?:any,pId?:any,versionId?:any){
        ...
        .catch(this.handleError)
	     ...
	     //获取项目分类树
    getItemTree(dictVal?: string,versionId?:any ) {

        let tempUrl = dictVal ? `${this.itemTreeUrl}?dictValue=${dictVal}` : this.itemTreeUrl+"?versionId="+versionId;
